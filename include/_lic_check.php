...
$myFile = fopen("../src/license.lic",'r');
    ...
    $id_key =$theData[1];
    $db_code = "";
    $active_key = mysqli_fetch_assoc(sqlsrv_query($conn,"select * from `activation_key` where `key_id`='$id_key'"));
    if($active_key!=""){
        ...
        if(password_verify($id, $db_code)){ 
            ...
            if(date("Y-m-d")<= $date){
                ...
                $users = (($ac_user/12)-57);
                $nunber_of_emp = (sqlsrv_query($conn,"select * from `eomploye_details`"));
