        ...
        // if($mac_id ==$sys_mac_id ){
            ...
            if(date("Y-m-d") <$date ){
                ...
                if(file_get_contents('license.lic') == ""){
                    ...
                    fclose($fp);
                    if(sqlsrv_query($conn,"insert into activationKeyDetails(keyId, activationKey, dateOfActivation, dateOfExpire, noOfUsers, status) values ('$key_id','$pass_key',GETUTCDATE(),'$date','$users', 'Active')")){

                        header("location:../index?i=y");
                    }
                    ...
                    $db_key = "";
                    $active_key = sqlsrv_fetch((sqlsrv_query($conn,"select * from activationKeyDetails where keyId='$id_key'")));
                    if($active_key!=""){
                        $db_key = $active_key['activationKey'];
                    }
